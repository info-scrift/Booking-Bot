...
//     const dayOfMonth = today.getDate()
//     if (dayOfMonth < 3) {
//       return new Response("Before 3rd of month; no reminders.", { status: 200 })
//     }
...
//       const hasCurrent = rows.some((r) => r.year === y && r.month === m)
//       if (!hasCurrent) {
//         const { data: inserted } = await supabase
//           .from("maintenance_payments")
//           .insert({
...
//       const totalDue = unpaid.reduce((sum, r) => sum + Number(r.amount || 0), 0)
//       const monthsList = unpaid.map((r) => `${String(r.month).padStart(2, "0")}-${r.year}`).join(", ")
//       const invoiceLink = `${APP_BASE_URL}/maintenance-invoice/${unpaid[0].id}?snapshot=unpaid`

//       const messageLines = [
//         "Reminder: Your maintenance payment is due.",
//         `Due months: ${monthsList}`,
//         `Total amount: Rs. ${totalDue.toLocaleString()}`,
//         `Invoice: ${invoiceLink}`,
//         "Please pay as soon as possible. Thank you.",
...
//       const totalDue = unpaid.reduce((sum, r) => sum + Number(r.amount || 0), 0)
//       const monthsList = unpaid.map((r) => `${String(r.month).padStart(2, "0")}-${r.year}`).join(", ")
//       const invoiceLink = `${APP_BASE_URL}/maintenance-invoice/${unpaid[0].id}`
